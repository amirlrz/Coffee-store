...
function BasketPage({basketdata}) {
    ...
    const {images,name,description,price,quantity} = basketdata
...
const strippedText = apiResponse.replace(/<[^>]*>/g, '');
 ...
 <Image width={96} height={96}  src={images[0].src} alt={name} />
 <div>
     <div className='bg-slate-200 text-lightorange rounded-full w-6 absolute left-[70px] mt-20 z-10'>{quantity}</div>
 <div onClick={()=>actions.addToBasket(basketdata)} className='bi bi-plus-circle-fill text-sm cursor-pointer text-rose-400 rounded-full absolute left-[91px] mt-[88px] z-10'></div>
 <div onClick={()=>actions.removeFromBasket(basketdata)} className='bi bi-dash-circle-fill cursor-pointer text-sm text-rose-400 rounded-full absolute left-[58px] mt-[88px] z-10'></div>
